...
df['Etiket'] = encoder.fit_transform(df['Durum'])
...
def analiz(vectorizer, veri, dosyaAdı):
    ...
    sonuçlarDf = pd.DataFrame({'Yorum': veri,})
    sonuçlarDf['Tahmin'] = encoder.inverse_transform(tahminSınıfları)
    sonuçlarDf.to_csv(dosyaAdı, index=False)


veriDf = pd.read_csv('C:/Users/berka/OneDrive/Masaüstü/yapayzeka/yorum/datasets/veri_seti_yorum.csv')
veri = veriDf['Yorum'].tolist()

analiz(vectorizer, veri, dosyaAdı='C:/Users/berka/OneDrive/Masaüstü/yapayzeka/yorum/datasets/analiz_sonuclari.csv')