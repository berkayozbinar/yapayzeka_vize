...
def tum_haberleri_cek(haber_sitesi_url):
    ...
    haberler = []
    for haber in soup.find_all("article", class_="p12-col"):
        url = haber.find("a", class_="box boxStyle hbBoxMainText")["href"]
        haberler.append(url)

    return haberler

def haber_metni_cek(haber_url):
    response = requests.get(haber_sitesi_url + haber_url)
    soup = BeautifulSoup(response.content, "html.parser")
...
haber_sitesi_url = "https://www.haberler.com/"
haber_url_listesi = tum_haberleri_cek(haber_sitesi_url)
...
for haber_url in haber_url_listesi:
    ...
    veri["Haber Metni"].append(haber_metni)

# DataFrame'i CSV dosyasına kaydetme
csv_dosya_yolu = "datasets/dataset.csv"
veri_df = pd.DataFrame(veri)
veri_df.to_csv(csv_dosya_yolu, index=False, encoding='utf-8')
